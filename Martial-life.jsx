...
const INITIAL_LOGS = [
  {
    ...
    title: "심지선관법 제1식",
    content: "긍정이 먼저요, 객관은 그 다음이다. 스트레스 없는 삶이 그 무엇보다 소중한 이치임을 깨달았다. 멘탈을 굳건히 한 뒤에야 세상을 바로 볼 수 있다.",
    innerPowerGain: 15,
  },
];
...
const INITIAL_DEMONS = [
  { id: 1, name: "미완성의 인연(未完成因緣)", desc: "끝나지도 시작되지도 않은 관계. 이 집착이 다음 경지로 가는 관문이다.", resolved: false },
];
...
function loadFromStorage(key, fallback) {
  ...
  const [demons, setDemons] = useState(() => loadFromStorage("ml_demons", INITIAL_DEMONS));
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ type: "enlightenment", title: "", content: "" });
  const [notification, setNotification] = useState(null);

  useEffect(() => { localStorage.setItem("ml_logs", JSON.stringify(logs)); }, [logs]);
  useEffect(() => { localStorage.setItem("ml_skills", JSON.stringify(skills)); }, [skills]);
  useEffect(() => { localStorage.setItem("ml_demons", JSON.stringify(demons)); }, [demons]);
  ...
  const progress = nextRealm
    ? Math.min(((totalPower - currentRealm.minPower) / (nextRealm.minPower - currentRealm.minPower)) * 100, 100)
    : 100;
  ...
  const submitLog = () => {
    ...
    const newLog = {
      ...
      innerPowerGain: gainMap[form.type],
    };
    setLogs(prev => [newLog, ...prev]);
    if (form.type === "enlightenment") {
      setSkills(prev => [
        { name: form.title, desc: form.content.slice(0, 55) + (form.content.length > 55 ? "…" : ""), level: 1, isNew: true },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d", color: "#e8e0d0", fontFamily: "'Noto Serif KR', 'Noto Serif', Georgia, serif" }}>
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", background: "radial-gradient(ellipse at 15% 15%, rgba(200,169,110,0.05) 0%, transparent 55%), radial-gradient(ellipse at 85% 85%, rgba(110,181,200,0.04) 0%, transparent 55%)" }} />
      {notification && (
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: "#1a1a1a", border: `1px solid ${typeConfig[notification.type].color}`, padding: "10px 22px", zIndex: 100, textAlign: "center", boxShadow: `0 0 24px ${typeConfig[notification.type].color}33` }}>
      ...
      <div style={{ borderBottom: "1px solid #1e1e1e", padding: "18px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, color: "#c8a96e", letterSpacing: 1 }}>{currentRealm.name}</div>
      ...
      <div style={{ padding: "20px", maxWidth: 560, margin: "0 auto" }}>
        {view === "profile" && (
          <div>
            <div style={{ border: "1px solid #222", padding: "24px 20px", marginBottom: 16, background: "#111", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: -20, right: -20, width: 100, height: 100, background: "radial-gradient(circle, rgba(200,169,110,0.07) 0%, transparent 70%)" }} />
              ...
              <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", padding: "14px" }}>
                ...
                <div style={{ height: 2, background: "#1e1e1e", borderRadius: 1 }}>
                  <div style={{ height: "100%", width: `${progress}%`, background: "linear-gradient(90deg, #c8a96e, #e8c98e)", borderRadius: 1, transition: "width 0.6s ease" }} />
                </div>
              ...
              <div style={{ border: "1px solid #222", padding: "18px", marginBottom: 16, background: "#111" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                  {Object.entries(typeConfig).map(([key, cfg]) => (
                    <button key={key} onClick={() => setForm(f => ({ ...f, type: key }))} style={{ padding: "9px", border: `1px solid ${form.type === key ? cfg.color : "#222"}`, background: form.type === key ? `${cfg.color}15` : "#0d0d0d", color: form.type === key ? cfg.color : "#555", fontSize: 11, letterSpacing: 1, cursor: "pointer", fontFamily: "inherit" }}>
                      {cfg.icon} {cfg.label}
                    </button>
                  ))}
                </div>
                <div style={{ fontSize: 10, color: "#555", marginBottom: 5, letterSpacing: 1 }}>제목</div>
                ...
                <div style={{ fontSize: 10, color: "#555", marginBottom: 5, letterSpacing: 1 }}>내용</div>
              ...
              <div key={i} style={{ border: `1px solid ${sk.isNew ? "#c8a96e44" : "#1e1e1e"}`, padding: "18px", marginBottom: 10, background: sk.isNew ? "#141209" : "#111", position: "relative" }}>
                ...
                <div style={{ display: "flex", gap: 3, marginBottom: 8 }}>
                  {Array.from({ length: 5 }).map((_, j) => (
                    <div key={j} style={{ width: 18, height: 2, background: j < sk.level ? "#c8a96e" : "#222" }} />
                  ))}
        ...
        {view === "demons" && (
          <div>
            ...
            <div style={{ border: "1px dashed #1e1e1e", padding: "18px", textAlign: "center", color: "#444", fontSize: 11, letterSpacing: 1, lineHeight: 1.8 }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus, textarea:focus { border-color: #c8a96e55 !important; }
